...
export class ModelBuilderAgentService {
    async generateModel(requirement: string, domain?: string): Promise<DeclarativeModel> {
        ...
        try {
            ...
            return parsed as DeclarativeModel;
        } catch (error: any) {
    ...
    private getStaticModelFallback(requirement: string, domain?: string): DeclarativeModel {
    ...
    private async callGemini(modelName: string, apiKey: string, requirement: string, domain?: string): Promise<Response> {
    ...
    private validateSchema(model: any) {
