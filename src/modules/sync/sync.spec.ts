...
import { DeviceDataService, SyncRegistryEntry, TelemetryReading } from "./device-data.service.js";
import { ThingsBoardClientService } from "./thingsboard-client.service.js";

describe("Sync Services Tests", () => {
    let mockDataService: any;
    let mockTbClient: any;
    let registryService: SyncRegistryService;
    let backgroundSyncService: BackgroundSyncService;

    // In-memory data store for testing
    let registryStore: Map<string, SyncRegistryEntry>;
    let readingsStore: TelemetryReading[];
    ...
    beforeEach(() => {
        ...
        mockDataService = {
            ...
            upsertRegistryEntry: vi.fn(async (deviceId: string, enabled: boolean, syncIntervalSeconds: number, lastSyncedTimestamp: number | null, lastSyncStatus: string | null, lastSyncError: string | null) => {
        ...
        mockTbClient = {
            getDeviceById: vi.fn(async (deviceId: string) => {
                if (deviceId === "invalid-device") {
                    throw new Error("Device not found");
                }
                return { id: { id: deviceId }, name: "Test Device" };
            }),
            getTelemetryKeys: vi.fn(async (deviceId: string) => {
                return ["temperature", "humidity"];
            }),
            getTelemetryRange: vi.fn(async (deviceId: string, keys: string[], startTs: number, endTs: number) => {
        ...
        registryService = new SyncRegistryService(
            mockDataService as unknown as DeviceDataService,
            mockTbClient as unknown as ThingsBoardClientService
        );
