...
export class RuleChainService {
    ...
    async createRuleChain(name: string, root: boolean = false, debugMode: boolean = false) {
        try {
            const existingId = await this.resolveRuleChainId(name);
            if (existingId) {
                const response = await axios.get(
                    `${TB_URL}/api/ruleChain/${existingId}`,
                    { headers: headers() }
                );
    ...
    async getRuleChain(ruleChainIdOrName?: string) {
        const id = await this.resolveRuleChainId(ruleChainIdOrName);
        const response = await axios.get(
            `${TB_URL}/api/ruleChain/${id}`,
            { headers: headers() }
        );
        return response.data;
    }

    async listRuleChains(pageSize: number = 10, page: number = 0) {
        const response = await axios.get(
            `${TB_URL}/api/ruleChains?pageSize=${pageSize}&page=${page}`,
            { headers: headers() }
        );
        return response.data;
    }

    async deleteRuleChain(ruleChainIdOrName: string) {
        const id = await this.resolveRuleChainId(ruleChainIdOrName);
        await axios.delete(
            `${TB_URL}/api/ruleChain/${id}`,
            { headers: headers() }
    ...
    async getRuleChainMetadata(ruleChainIdOrName?: string) {
        const id = await this.resolveRuleChainId(ruleChainIdOrName);
        const response = await axios.get(
            `${TB_URL}/api/ruleChain/${id}/metadata`,
            { headers: headers() }
        );
        return response.data;
    }

    async saveRuleChainMetadata(metadata: any) {
    ...
    async getRuleNodeCatalog(category?: string) {
        const response = await axios.get(
    ...
    async addNodeToRuleChain(
        ...
        const duplicate = metadata.nodes.find((n: any) => n.name.toLowerCase() === nodeName.toLowerCase());
        if (duplicate) {
        ...
        if (component.clazz === "org.thingsboard.rule.engine.notification.TbNotificationNode") {
            if (!mergedConfig.targets || (Array.isArray(mergedConfig.targets) && mergedConfig.targets.length === 0) || mergedConfig.targets === null) {
                try {
                    ...
                    const allUsersTarget = list.find((t: any) => t.name.toLowerCase() === "all users") || list[0];
                    if (allUsersTarget) {
            ...
            if (Array.isArray(mergedConfig.targets)) {
                mergedConfig.targets = mergedConfig.targets.map((t: any) => typeof t === "object" ? (t.id ?? t) : t);
            }
        ...
        const savedNode = (savedMetadata.nodes ?? []).find(
            (n: any) => n.name.toLowerCase() === nodeName.toLowerCase()
        );
    ...
    async connectNodesInRuleChain(
        ruleChainIdOrName: string | undefined,
        fromNodeName: string,
        toNodeName: string,
        relationType: string = "Success"
    ) {
        const ruleChainId = await this.resolveRuleChainId(ruleChainIdOrName);
        ...
        const fromIndex = metadata.nodes.findIndex((n: any) =>
            n.name.toLowerCase() === fromNodeName.toLowerCase() ||
            (n.id?.id && n.id.id.toLowerCase() === fromNodeName.toLowerCase())
        );
        const toIndex = metadata.nodes.findIndex((n: any) =>
            n.name.toLowerCase() === toNodeName.toLowerCase() ||
        ...
        const exists = metadata.connections.some((c: any) =>
            c.fromIndex === fromIndex && c.toIndex === toIndex && c.type === relationType
        );
    ...
    async listNotificationTargets(pageSize: number = 10, page: number = 0) {
        const response = await axios.get(
            `${TB_URL}/api/notification/targets?pageSize=${pageSize}&page=${page}`,
            { headers: headers() }
        );
        return response.data;
    }

    async listNotificationTemplates(pageSize: number = 10, page: number = 0) {
        const response = await axios.get(
            `${TB_URL}/api/notification/templates?pageSize=${pageSize}&page=${page}`,
            { headers: headers() }
        );
        return response.data;
    }
    ...
    async resolveRuleChainId(ruleChainIdentifier?: string): Promise<string> {
        ...
        const exactMatch = list.find((rc: any) => rc.name.toLowerCase() === ruleChainIdentifier.toLowerCase());
        if (exactMatch) {
            return exactMatch.id.id;
        }

        const fuzzyMatch = list.find((rc: any) => rc.name.toLowerCase().includes(ruleChainIdentifier.toLowerCase()));
        if (fuzzyMatch) {
