...
export class RuleChainTools {
    ...
    async createRuleChain(
        input: { name: string; debugMode?: boolean },
    ...
    async getRuleChainMetadata(
        ...
        try {
            ...
            const nodes = (metadata.nodes ?? []).map((n: any) => ({
                id: n.id?.id,
                name: n.name,
                type: n.type,
            ...
            const connections = (metadata.connections ?? []).map((c: any) => ({
    ...
    async connectRuleNodes(
        ...
        try {
            const result = await service.connectNodesInRuleChain(
                input.ruleChain,
                input.fromNode,
                input.toNode,
                input.relation
    ...
    async listNotificationTemplates(
        ...
        try {
            ...
            return {
                success: true,
                templates: (result.data ?? []).map((t: any) => ({
                    id: t.id?.id,
                    name: t.name,
