    ...
    mappings: [
        {
            metric: "RPM",
            targetRole: "impeller",
            property: "rotationSpeed" as const,
...
describe("3D Visualization Component Tests", () => {
    ...
    describe("validateVisualMapping", () => {
        ...
        it("should reject a mapping whose targetRole doesn't match any defined part role", () => {
            ...
            expect(() => validateVisualMapping(invalidMapping, mockTelemetrySchema)).toThrow(
                /targets role 'non_existent_role' which does not exist/
            );
        });
    ...
    describe("preview_visual_mapping Tool", () => {
        it("should produce valid output using midpoint values with no real device present", async () => {
            ...
            const tools = new VisualizationTools(
                mockMappingService as any,
            ...
            const result = await tools.previewVisualMapping({ deviceType: "centrifugal_pump" }, ctx as any);
