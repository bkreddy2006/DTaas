...
export class ThingsBoardTools {
    ...
    async createEmulator(input, ctx) {
        try {
            ...
            const emulator = await service.createEmulatorDevice(input.deviceName, input.emulatorType ?? "smart-home-energy-hub", input.scenario ?? "Typical Day", input.telemetryRateSeconds ?? 5);
            const deviceId = emulator.device.id.id;
            // Create dashboard
            const dashboard = await dashboardService.createDashboard(`${input.deviceName} Dashboard`);
            // Add widget
            const widget = await dashboardService.addSmartWidget(dashboard.id.id, deviceId, input.deviceName);
            return {
                success: true,
                emulator,
                dashboard,
    ...
    async configureCredentials(input, ctx) {
...
__decorate([
    ...
    __metadata("design:paramtypes", [Object, Object]),
    __metadata("design:returntype", Promise)
], ThingsBoardTools.prototype, "configureCredentials", null);
__decorate([
    Tool({
