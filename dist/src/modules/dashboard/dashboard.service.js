 ...
 */
function findNextFreePosition(widgets, sizeX, sizeY, gridCols = 24) {
    let r = 0;
    while (true) {
 ...
 */
function pickWidgetKind(keys) {
    const lower = keys.map(k => k.toLowerCase());
    if (lower.some(k => k.includes("alarm") || k.includes("alert")))
        return "alarm_table";
    if (keys.length === 1 && lower.some(k => k.includes("level") || k.includes("percent") || k.includes("battery") || k.includes("fill")))
        return "gauge";
    if (keys.length === 1)
        return "value_card";
 ...
 */
async function fetchWidgetDescriptor(kind) {
    const { bundleAlias, typeAlias } = WIDGET_TYPES[kind];
    const res = await axios.get(`${TB_URL}/api/widgetType?bundleAlias=${bundleAlias}&alias=${typeAlias}&isSystem=true`, { headers: headers() });
...
export class DashboardService {
    ...
    async createDashboard(title) {
        try {
            ...
            if (existingId) {
                const response = await axios.get(`${TB_URL}/api/dashboard/${existingId}`, { headers: headers() });
        ...
        const response = await axios.post(`${TB_URL}/api/dashboard`, {
            title,
            configuration: {
                widgets: {},
                states: {
    ...
    async getDashboard(dashboardIdOrName) {
        const dashboardId = await this.resolveDashboardId(dashboardIdOrName);
        const response = await axios.get(`${TB_URL}/api/dashboard/${dashboardId}`, { headers: headers() });
        return response.data;
    }
    async resolveDashboardId(dashboardIdentifier) {
        ...
        // Exact match
        const exactMatch = list.find((d) => d.title.toLowerCase() === dashboardIdentifier.toLowerCase());
        if (exactMatch) {
            return exactMatch.id.id;
        }
        // Fuzzy match
        const fuzzyMatch = list.find((d) => d.title.toLowerCase().includes(dashboardIdentifier.toLowerCase()));
        if (fuzzyMatch) {
            return fuzzyMatch.id.id;
        }
        throw new Error(`Could not find dashboard with name or ID matching "${dashboardIdentifier}".`);
    }
    async resolveWidgetId(dashboardId, widgetIdentifier) {
    ...
    async listDashboards(pageSize = 10, page = 0) {
        const response = await axios.get(`${TB_URL}/api/user/dashboards?pageSize=${pageSize}&page=${page}`, { headers: headers() });
        return response.data;
    }
    async deleteDashboard(dashboardIdOrName) {
        const dashboardId = await this.resolveDashboardId(dashboardIdOrName);
        await axios.delete(`${TB_URL}/api/dashboard/${dashboardId}`, { headers: headers() });
    }
    // ── Smart widget add ──────────────────────────────────────────────────────
    async addSmartWidget(dashboardId, deviceId, widgetTitle, telemetryKeysInput) {
        let telemetryKeys = telemetryKeysInput ?? [];
        if (telemetryKeys.length > 0) {
            ...
            await axios.post(`${TB_URL}/api/plugins/telemetry/DEVICE/${deviceId}/timeseries/ANY`, dummyData, { headers: headers() });
        }
        else {
            // 1. Fetch the device's telemetry keys from ThingsBoard
            let keysRes = await axios.get(`${TB_URL}/api/plugins/telemetry/DEVICE/${deviceId}/keys/timeseries`, { headers: headers() });
            ...
            if (telemetryKeys.length === 0) {
                ...
                await axios.post(`${TB_URL}/api/plugins/telemetry/DEVICE/${deviceId}/timeseries/ANY`, randomTelemetry, { headers: headers() });
                await new Promise(resolve => setTimeout(resolve, 1000));
                keysRes = await axios.get(`${TB_URL}/api/plugins/telemetry/DEVICE/${deviceId}/keys/timeseries`, { headers: headers() });
                telemetryKeys = keysRes.data;
            }
        }
        // 2. Pick the best widget type based on key names + count
        const kind = pickWidgetKind(telemetryKeys);
        const { bundleAlias, typeAlias, type, defaultConfig } = await fetchWidgetDescriptor(kind);
        // 3. Fetch the existing dashboard
        const dashboard = await this.getDashboard(dashboardId);
        const config = dashboard.configuration ?? {};
        if (!config.widgets)
            config.widgets = {};
        if (!config.entityAliases)
        ...
        if (!mainLayout.widgets)
            mainLayout.widgets = {};
        // Read the existing grid column count so our sizeX values are correct
        const gridCols = mainLayout.gridSettings?.columns ?? 24;
        ...
        const { row, col } = findNextFreePosition(mainLayout.widgets, dims.sizeX, dims.sizeY, gridCols);
        ...
        const saveRes = await axios.post(`${TB_URL}/api/dashboard`, dashboard, { headers: headers() });
        return {
    ...
    async deleteWidget(dashboardIdOrName, widgetIdOrTitle) {
        const dashboardId = await this.resolveDashboardId(dashboardIdOrName);
        ...
        dashboard.configuration = config;
        await axios.post(`${TB_URL}/api/dashboard`, dashboard, { headers: headers() });
    }
    async updateWidgetLayout(dashboardIdOrName, widgetIdOrTitle, layout) {
        const dashboardId = await this.resolveDashboardId(dashboardIdOrName);
        const widgetId = await this.resolveWidgetId(dashboardId, widgetIdOrTitle);
        const dashboard = await this.getDashboard(dashboardId);
        const config = dashboard.configuration ?? {};
